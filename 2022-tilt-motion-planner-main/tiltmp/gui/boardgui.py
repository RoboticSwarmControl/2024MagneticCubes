        ...
        def draw_tile(t):
            if t in board.fixed_tiles:
                board.rectangles.append(
                    canvas.create_rectangle(tilesize * t.x, tilesize * t.y, tilesize * t.x + tilesize,
                                            tilesize * t.y + tilesize, fill=t.color, width=2))
            else:
                board.rectangles.append(
                    canvas.create_rectangle(tilesize * tile.x, tilesize * tile.y, tilesize * tile.x + tilesize,
                                            tilesize * tile.y + tilesize, fill=t.color))
    else:
        def draw_tile(t):
            board.rectangles.append(
                canvas.create_rectangle(tilesize * tile.x, tilesize * tile.y, tilesize * tile.x + tilesize,
    ...
    for p in board.polyominoes:
        for tile in p.get_tiles():
            ...
            if tile.glues[0] != "None":
                #north
                board.glueText.append(canvas.create_text(tilesize*tile.x + tilesize//2, tilesize*tile.y + tilesize//5, text = tile.glues[0], fill=textcolor, font=('',tilesize//4)) )
            if tile.glues[1] != "None": 
                #east
                board.glueText.append(canvas.create_text(tilesize*tile.x + tilesize - tilesize//5, tilesize*tile.y + tilesize//2, text = tile.glues[1], fill=textcolor, font=('',tilesize//4)))
            if tile.glues[2] != "None": 
                #south
                board.glueText.append(canvas.create_text(tilesize*tile.x + tilesize//2, tilesize*tile.y+ tilesize - tilesize//5, text = tile.glues[2], fill=textcolor, font=('',tilesize//4) ))
            if tile.glues[3] != "None": 
                #west
                board.glueText.append(canvas.create_text(tilesize*tile.x + tilesize//5, tilesize*tile.y + tilesize//2, text = tile.glues[3], fill=textcolor, font=('',tilesize//4) ))

    for x,y in board.get_concrete_positions():
        canvas.create_rectangle(tilesize*x, tilesize*y, tilesize*x + tilesize, tilesize*y + tilesize, fill = "#686868")
    ...
    if b_drawLoc == True:
        for row in range(boardheight):
            for col in range(boardwidth):
                canvas.create_text(tilesize*(col+1) - tilesize//2, tilesize*(row+1) - tilesize//2, text = "("+str(col)+","+str(row)+")", fill=gridcolor,font=('',tilesize//4))
