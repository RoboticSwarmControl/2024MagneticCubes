...
def parseFile(filename):
    ...
    for tile in tile_set_data["tileData"]:
        if tile["concrete"] != "True":
            glues = TT.Glues(tile["northGlue"], tile["eastGlue"], tile["southGlue"], tile["westGlue"])
            t = TT.Tile(color=tile["color"], glues=glues, position=(tile["location"]["x"], tile["location"]["y"]))
            board.add(TT.Polyomino(tiles=[t]))
        else:
            t = TT.Tile(color=tile["color"], is_concrete=True, position=(tile["location"]["x"], tile["location"]["y"]))
            board.add_concrete(t.x, t.y)

    for prevTile in prevTiles:
        prevGlues = TT.Glues(prevTile["northGlue"], prevTile["eastGlue"], prevTile["southGlue"],
                            prevTile["westGlue"])
        prevTileList.append(TT.Tile(glues=prevGlues, color=prevTile["color"], is_concrete=prevTile["concrete"]))
