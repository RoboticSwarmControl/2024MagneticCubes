...
from tiltmp.mp.motionplanner import Instance, get_motion_planner, OneTileAtATimeMotionPlanner, MotionPlanner, \
    get_anchoring_motion_planner, BFSMotionPlanner
...
class SaveSVGDialog(tkinter.Toplevel):
    def __init__(self, parent):
        ...
        self.scale_label = tkinter.Label(self.scale_container, text="SVG scale", )
        self.scale_label.pack(side=tkinter.LEFT)
        self.scale_var = IntVar()
        self.scale_var.set(1)
        self.scale = tkinter.Entry(self.scale_container, textvariable=self.scale_var, width=5)
        ...
        self.filename_box = tkinter.Entry(self.select_file_container, state=tkinter.DISABLED,
                                          textvariable=self.filename_var, width=30)
        self.filename_box.pack(side=tkinter.LEFT, padx=5)
        self.select_file_button = tkinter.Button(self.select_file_container, text="Select",
        ...
        self.border_checkbox = tkinter.Checkbutton(self.checkbox_container, text="Border",
                                                   variable=self.border_checkvar,
                                                   onvalue=1, offvalue=0)
        self.border_checkbox.pack(side=tkinter.LEFT)
        self.gridlines_checkbox = tkinter.Checkbutton(self.checkbox_container, text="Gridlines",
                                                      variable=self.gridlines_checkvar,
                                                      onvalue=1, offvalue=0)
        self.gridlines_checkbox.pack(side=tkinter.LEFT)
        self.checkbox_container.pack(padx=5, pady=5)

        self.save_button = Button(self.checkbox_container, text="Save", command=lambda: self.save(), width=12)
    ...
    def save(self):
        try:
            ...
            tkinter.messagebox.showerror("Error", "Scale needs to be an integer", parent=self)
            return
        try:
            self.parent.board2SVG(self.parent.board, filename=self.filename_var.get(),
                                  gridlines=self.gridlines_checkvar.get(), border=self.border_checkvar.get(),
                                  scale=scale)
...
class SequenceToSVGDialog(tkinter.Toplevel):
    def __init__(self, parent):
        ...
        self.scale_label = tkinter.Label(self.scale_and_prefix_container, text="SVG scale")
        self.scale_label.pack(side=tkinter.LEFT)
        self.scale_var = IntVar()
        self.scale_var.set(1)
        self.scale = tkinter.Entry(self.scale_and_prefix_container, textvariable=self.scale_var, width=5)
        self.scale.pack(side=tkinter.LEFT, padx=(0, 20))
        self.prefix_label = tkinter.Label(self.scale_and_prefix_container, text="Prefix")
        self.prefix_label.pack(side=tkinter.LEFT)
        self.prefix_var = StringVar()
        self.prefix_var.set("img")
        self.prefix = tkinter.Entry(self.scale_and_prefix_container, textvariable=self.prefix_var)
        ...
        self.sequence = tkinter.Entry(self.sequence_container, textvariable=self.sequence_var, width=30)
        ...
        self.directory_box = tkinter.Entry(self.select_directory_container, state=tkinter.DISABLED,
                                           textvariable=self.directory_var, width=30)
        self.directory_box.pack(side=tkinter.LEFT, padx=5)
        self.select_file_button = tkinter.Button(self.select_directory_container, text="Select Directory",
    ...
    def save(self):
        try:
            ...
            tkinter.messagebox.showerror("Error", "Sequence can only contain the characters NESW", parent=self)
            return
        try:
            scale = self.scale_var.get()
        except:
            ...
            return
        try:
            self.parent.sequence_to_svgs(sequence, self.directory_var.get(),
...
class MsgAbout:
    def __init__(self, parent):
        ...
        Label(self.t, text="Developed for my bachelors thesis.\nBased on the Tumble Tiles software:").pack()

        def link_callback(url):
            import webbrowser
            webbrowser.open_new(url)

        link1 = Label(self.t, text="https://github.com/asarg/TumbleTiles", fg="blue", cursor="hand2")
        link1.pack()
        link1.bind("<Button-1>", lambda e: link_callback("https://github.com/asarg/TumbleTiles"))
...
class VideoExport:
    def __init__(self, parent, tumblegui):  # , fun):
        ...
        self.progress = tkinter.ttk.Progressbar(self.t, orient=HORIZONTAL, variable=self.progress_var, length=260,
                                                mode='determinate')
...
class MotionPlannerDialog(tkinter.Toplevel):
    def __init__(self, parent, title, messages, yesno=False):
        ...
        if yesno:
            self.yes_button = tkinter.Button(self, text="Yes", command=self.on_yes, width=10)
            self.yes_button.pack(side="right", padx=5, pady=5)
            self.no_button = tkinter.Button(self, text="No", command=self.on_no, width=10)
            self.no_button.pack(side="right", padx=5, pady=5)

        else:
            self.ok_button = tkinter.Button(self, text="OK", command=self.on_ok, width=10)
...
class MotionPlannerFrame(tkinter.Frame):
    def __init__(self, main_gui, *args, **kwargs):
        ...
        self.txt = MotionPlannerLogTextBox(self, self.main_gui.motion_planner_log, width=42)
        self.txt.pack(side=TOP)
        self.running_info.pack(side=TOP)
        self.stop_button = Button(self, text="Stop", width=15,
                                  command=self.main_gui.stop_motion_planner)
...
class StoppableMotionPlannerThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            tkinter.messagebox.showinfo("Motion Planner Error", str(traceback.format_exc()))
            self._exit()
            return
        end = time.time()
        if self.stopped:
            self.gui.motion_planner_log.append(self.name + " Motion Planner\n" +
                                               "Search stopped.\nTime used:\t\t\t{:.2f} s".format(end - start))
            self._exit()
            MotionPlannerDialog(self.gui.mainframe, "Search stopped", self.gui.motion_planner_log)
            return
            ...
            self.gui.motion_planner_log.append(self.name + " Motion Planner\n" +
                                               ...
                                               "Time used:\t\t\t{:.2f} s".format(end - start))
            self._exit()
            MotionPlannerDialog(self.gui.mainframe, "Search finished", self.gui.motion_planner_log)
            return
        ...
        self.gui.motion_planner_log.append(self.name + " Motion Planner\n" +
                                           ...
                                           "Time used:\t\t\t{:.2f} s".format(end - start))
        self._exit()
        dialog = MotionPlannerDialog(self.gui.mainframe, "Run Sequence?", self.gui.motion_planner_log, yesno=True)
        if dialog.get():
...
class tumblegui:
    def __init__(self, root):
        ...
        self.filemenu.add_command(
            label="Create SVG",
            command=lambda: SaveSVGDialog(self))
        # command=lambda: self.board2SVG(self.board))
        self.filemenu.add_command(
            label="Sequence to SVGs",
            command=lambda: SequenceToSVGDialog(self))
        ...
        self.filemenu.add_command(
            ...
            command=lambda: self.reload_file())
       ...
       #     command=self.setFactoryMode)
        ...
        self.motion_planner_menu.add_command(
            label="Run Motion Planner",
            command=self.run_motion_planner)

        heuristic_menu = Menu(self.menubar)
        self.heuristic_var = StringVar(value=next(iter(HEURISTICS.keys())))
        for name in HEURISTICS:
            heuristic_menu.add_radiobutton(label=name, value=name, variable=self.heuristic_var)
        ...
        mode_menu.add_radiobutton(label="Polyomino Construction",
                                  value="Polyomino Construction",
                                  variable=self.mode_var)
        mode_menu.add_radiobutton(label="Anchoring",
                                  value="Anchoring",
                                  variable=self.mode_var)
        ...
        self.single_tile_heuristic_var = StringVar(value=next(iter(SINGLE_TILE_HEURISTICS.keys())))
        for name in SINGLE_TILE_HEURISTICS:
            single_tile_heuristic_menu.add_radiobutton(label=name, value=name, variable=self.single_tile_heuristic_var)
        self.motion_planner_menu.add_cascade(label="Single Tile Heuristic", menu=single_tile_heuristic_menu)

        self.motion_planner_menu.add_separator()

        self.motion_planner_menu.add_command(
            label="Run RRT Motion Planner",
            command=self.run_rrt_motion_planner)

        self.motion_planner_menu.add_separator()

        self.motion_planner_menu.add_command(
            label="Stop Motion Planner",
            command=self.stop_motion_planner
        )

        self.motion_planner_menu.add_command(
            label="Show Log",
            command=self.show_motion_planner_log
        )
        ...
        self.sequence_menu.add_command(
            ...
            command=self.record_sequence
        )
        self.sequence_menu.add_command(
            label=self.runScriptText,
            command=self.enter_sequence
        )
        self.sequence_menu.add_command(
            label="Run from File",
            command=self.run_sequence_from_file
        )
        ...
        self.execution_speed_menu.add_radiobutton(label="No delay", variable=self.script_speed_var, value=0.0)
        self.execution_speed_menu.add_radiobutton(label="Fast", variable=self.script_speed_var, value=0.1)
        self.execution_speed_menu.add_radiobutton(label="Normal", variable=self.script_speed_var, value=0.25)
        self.execution_speed_menu.add_radiobutton(label="Slow", variable=self.script_speed_var, value=0.5)
        self.execution_speed_menu.add_radiobutton(label="Very Slow", variable=self.script_speed_var, value=1.0)
        self.sequence_menu.add_cascade(
            label="Execution Speed",
            menu=self.execution_speed_menu
        )
        ...
        self.motion_planner_frame = MotionPlannerFrame(self, self.rightFrame, relief=FLAT)
    ...
    def set_active_motion_planner(self, mp, name):
        ...
        self.motion_planner_frame.running_info["text"] = name + " motion planner running"
    ...
    def enter_sequence(self):
        ...
        try:
            ...
            tkinter.messagebox.showerror("Error", "Sequence can only contain the characters NESW")
            return
    ...
    def parse_sequence(self, input_string):
        ...
        if any(letter not in ["W", "E", "N", "S"] for letter in sequence):
            raise ValueError("Sequence can only Sequence can only contain the characters NESW")
        return sequence
    ...
    def run_sequence(self, sequence):
        if self.sequence_execution_thread is not None:
            return
        self.sequence_execution_thread = StoppableSequenceExecutionThread(sequence, self)
    ...
    def resizeBoardAndCanvas(self):
        ...
        toolbarframeheight = 24
        self.root.geometry(str(self.board.cols * TILESIZE + 500) + 'x' +
                           str(self.board.rows * TILESIZE + toolbarframeheight + 30))
        self.w.config(
            width=self.board.cols *
                  TILESIZE,
    ...
    def generate_original_board(self):
        if not hasattr(self, "obg") or self.obg is None or self.board is not self.ogb_board:
            try:
        ...
        try:
            ...
            self.callCanvasRedraw()
            tkinter.messagebox.showinfo("Reverse Stepper", "Displayed all possible reverse steps")
            self.obg = None
            return
        self.callCanvasRedraw()
    ...
    def load_instance(self):
        filename = tkinter.filedialog.askopenfilename(filetypes=(("json files", "*.json"), ("all files", "*")))
    ...
    def run_one_tile_at_a_time_motion_planner(self):
        ...
        mp = OneTileAtATimeMotionPlanner(Instance(self.board, self.target_shape),
                                         single_tile_heuristic=heuristic)
        self.motion_planner_thread = StoppableMotionPlannerThread(mp, "Single Tile " + h_name, self)
        self.motion_planner_thread.start()
    ...
    def run_motion_planner(self):
        if self.target_shape is None:
            return
        if self.motion_planner_thread is not None:
            tkinter.messagebox.showinfo(title="Tumble Tiles",
        ...
        if self.mode_var.get() == "Polyomino Construction":
            ...
            mp = get_motion_planner(Instance(self.board, self.target_shape), heuristic=heuristic)
        elif self.mode_var.get() == "Anchoring":
            mp = get_anchoring_motion_planner(Instance(self.board, self.target_shape))

        self.motion_planner_thread = StoppableMotionPlannerThread(mp, self.heuristic_var.get(), self)
        self.motion_planner_thread.start()
    ...
    def show_motion_planner_log(self):
        MotionPlannerDialog(self.mainframe, "Motion Planner Log", self.motion_planner_log)
    ...
    def setTilesFromEditor(self, board, glueFunc, prev_tiles, width, height, target_shape):
    ...
    def sequence_to_svgs(self, sequence, directory, prefix="img", gridlines=False, border=True, scale=1):
        from math import log
        path = os.path.join(directory, prefix)
        suffix_length = math.ceil(log(len(sequence), 10))
        self.board2SVG(self.board, filename=path + ("0" * suffix_length) + ".svg",
                       gridlines=gridlines, border=border, scale=scale)
        i = 0
        for step in sequence:
            i += 1
            self.MoveDirection(step)
            filename = (path + "{:0" + str(suffix_length) + "d}.svg").format(i)
            self.board2SVG(self.board, filename=filename, gridlines=gridlines, border=border, scale=scale)

    def board2SVG(self, board: Board, filename="test.svg", gridlines=False, border=True, scale=1):
        ...
        f.write('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" baseProfile="full" width="' +
                str(w + 2 * scale) + '" height="' + str(h + 2 * scale) + '">\n')
...
""")
        ...
        if border:
            border = [(0, y) for y in range(board.rows + 1)] + [(board.cols + 1, y) for y in range(board.rows + 1)] + \
                     [(x, 0) for x in range(board.cols + 1)] + [(x, board.rows + 1) for x in range(board.cols + 2)]
            c = "#808080"
            for x, y in border:
                x = scale * x
                y = scale * y
                line = '<rect x="' + str(x) + '" y="' + str(y) + '" width="' + str(
                    scale) + '" height="' + str(scale) + '" fill="' + str(
                    c) + '" stroke="black" stroke-width="' + str(0.05 * scale) + '"/>\n'
                f.write(line)
        ...
        if gridlines:
            for x in range(board.cols):
                for y in range(board.rows):
                    c = "#ffffff"
                    line = '<rect x="' + str((x + 1) * scale) + '" y="' + str((y + 1) * scale) + '" width="' + str(
                        scale) + '" height="' + str(
                        scale) + '" fill="' + str(c) + '" stroke="black" stroke-width="' + str(
                        0.05 * scale) + '" fill-opacity="0" />\n'
                    f.write(line)
        # tiles
        for tile in board.get_tiles():
            ...
            c = tile.color
            line = '<rect x="' + str((x + 1) * scale) + '" y="' + str((y + 1) * scale) + '" width="' + str(
                scale) + '" height="' + str(scale) + '" fill="' + str(c) + '" stroke="black" stroke-width="' + str(
                0.05 * scale) + '"/>\n'
            f.write(line)
        ...
        if self.target_shape is not None:
            for x, y in [(t.x, t.y) for t in self.target_shape.get_tiles()]:
                line = '<rect x="' + str((x + 1) * scale) + '" y="' + str((y + 1) * scale) + '" width="' + str(
                    scale) + '" height="' + str(scale) + '" stroke="black" stroke-width="0.0" fill-opacity="' + str(
                    0.4) + \
        ...
        if gridlines:
            for x in range(w / scale):
                for y in range(h / scale):
                    c = "#ffffff"
                    line = '<rect x="' + str(x * scale + 1) + '" y="' + str(y * scale + 1) + '" width="' + str(
                        scale) + '" height="' + str(
    ...
    def CreateInitial(self):
        try:
            self._load_instance("../Testcases/easyexample.json")
        ...
        for i in range(NumTiles):
            ...
            colorb = "#" + str(hex(random.randint(0,
                                                  16))[2:]) + str(hex(random.randint(0,
                                                                                     16))[2:]) + str(
                hex(random.randint(0,
                                   16))[2:])
            if len(colorb) > 4:
                colorb = colorb[:4]
            t = TT.Tile(position=(bh - i - 2, bh - 1), glues=TT.Glues('N', 'E', 'S', 'W'), color=colorb)
            ...
            char = chr(ord('a') + i)
            t = TT.Tile(position=(0, bh - i - 2), glues=TT.Glues('S', 'W', 'N', 'E'), color=colorb)
            p = TT.Polyomino(tiles=[t])
    ...
    def redraw_target_shape(self):
        ...
        self.target_shape_rectangles = [self.w.create_rectangle(TILESIZE * t.x, TILESIZE * t.y,
                                                                TILESIZE * (t.x + 1), TILESIZE * (t.y + 1)
                                                                , outline="Red")
