...
from tiltmp.core.gridcreation import random_cave_board, random_maze_board, random_maze_board_no_chambers
...
class TileEditorGUI:
    def __init__(self, parent, tumbleGUI, board, glue_data, previewTileList):
        ...
        optionsMenu.add_command(label="Reachable Set", command=lambda: self.drawReachableSet())
        ...
        newBoardMenu.add_command(label="Random Maze", command=lambda: self.newMazeBoard())
        newBoardMenu.add_command(label="Random Maze without open areas", command=lambda: self.newMazeBoardNoChambers())
        newBoardMenu.add_command(label="Random Caves", command=lambda: self.newCaveBoard())


        motionPlannerMenu = Menu(self.menuBar, tearoff=0)
        motionPlannerMenu.add_command(label="Load Instance", command=lambda: self.load_instance())
        motionPlannerMenu.add_command(label="Save Instance", command=lambda: self.save_instance())
...
Shift + Right-Click:
    ...
    def populateArray(self):
        ...
        for x, y in self.board.get_concrete_positions():
            self.coord2tile[x][y] = None #TODO change representation of concrete in the editor
    ...
    def selectionVerticallyFlipped(self):
        ...
        for x in range(0, selectionWidth):
            for y in range(0, selectionHeight):
                ...
                newConcTile = TT.Tile(position=(newX, newY), glues=glues, color=color, is_concrete=True)
    ...
    def drawTargetSelection(self):
        self.targetSelection = [self.BoardCanvas.create_rectangle(self.tile_size*x, self.tile_size*y,
                                                                  self.tile_size*(x+1), self.tile_size*(y+1),
                                                                  outline="Red")
    ...
    def drawReachableSet(self):
        ...
        if not self.target_shape:
            return
        self.clearTargetSelection()
        p = TT.Polyomino(tiles=[TT.Tile(position=pos) for pos in sorted(self.target_shape)])
        left, up = p.position[0], p.position[1]
        self.reachableOverlay = [
            self.BoardCanvas.create_rectangle(self.tile_size * x,
                                              self.tile_size * y,
                                              self.tile_size * (x + 1), self.tile_size * (y + 1),
                                              fill="#00dd00")
            for x, y in reachable_set(self.board, p)]
        self.reachableOverlay += [self.BoardCanvas.create_rectangle(self.tile_size * left,
                                              self.tile_size * up,
                                              self.tile_size * (left + 1), self.tile_size * (up + 1),
                                              fill="#dd0000")]
    ...
    def exportTiles(self):
        ...
        self.tumbleGUI.setTilesFromEditor(self.board, self.glue_data, self.prevTileList, self.board.cols,
                                          self.board.rows, self.target_shape)
    ...
    def load_instance(self):
        file = tkinter.filedialog.askopenfilename(parent=self.newWindow,
                                                  filetypes=(("json files","*.json"),("all files","*")))
        instance = read_instance(file)
        del self.board.polyominoes[:]
        self.board.LookUp = {}
        self.board = instance.initial_state
        self.target_shape = set((tile.x, tile.y) for tile in instance.target_shape.tiles.values())
        self.redrawPrev()
        self.clearTargetSelection()
        self.drawTargetSelection()
