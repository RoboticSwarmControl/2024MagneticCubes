...
class NotEnoughTilesPruner(Pruner):
    ...
    def setup(self, board: Board, target: Polyomino):
        ...
        self.target_area = {(pos[0] + dx, pos[1] + dy) for dx, dy in self.target_shape.get_shape()[1]}
...
class PackingNoLeftoversPruner(Pruner):
    ...
    def is_prunable(self, changed):
        ...
        return not is_packable(self.target_shape.get_shape()[1],
                               [p.get_shape()[1] for p in largest])
...
class WrongTilesCombinedPruner(Pruner):
    ...
    def is_prunable(self, changed):
        ...
        if not self.mp.poly in self.mp.board.polyominoes:
            ...
            if not (previous_corner.x + self.mp.destination[0] == self.mp.tile.x and
                    previous_corner.y + self.mp.destination[1] == self.mp.tile.y):
        ...
        for p in changed:
            ...
            if not (self.poly_size + 1 == p.size):
                # additional tiles combined with the target polyomino
                return True
            if not (p.position[0] + self.mp.destination[0] == self.mp.tile.x and
                    p.position[1] + self.mp.destination[1] == self.mp.tile.y):
                # tile is in the wrong place
                return True
...
class TargetUnreachablePruner(Pruner):
    def __init__(self, motion_planner):
        ...
        dx = self.mp.target_shape.position[0] + self.mp.offset[0] - self.target_poly.position[0]
        dy = self.mp.target_shape.position[1] + self.mp.offset[1] - self.target_poly.position[1]
