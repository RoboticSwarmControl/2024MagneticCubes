...
from tiltmp.core.algorithms import center, find_shortest_path, nearest_tile, distance_to_area, reachable_set, \
    compute_distances, compute_distance_within_set, breadth_first_distance
...
class DistanceBasedHeuristic(ABC):
    def __init__(self, motion_planner, precomputed_distances=None):
        ...
        if precomputed_distances is not None and "precomputed_distances" in precomputed_distances:
    ...
    def _n_closest(self):
        if self._tiles_needed == len(self._board.get_tiles()):
            ...
            return heapq.nsmallest(self._tiles_needed, self._board.get_tiles(),
                                   key=lambda t: self._distances[t.x, t.y] if t.parent.can_reach else float("inf"))
...
class AverageDistanceHeuristic(DistanceBasedHeuristic):
    ...
    def __call__(self, score):
        ...
        if len(n_closest) < self._tiles_needed:
            return float("inf")
        return sum(self._distances[(t.x, t.y)] for t in n_closest) / self._tiles_needed + score
...
class AverageDistanceAnchoringHeuristic(DistanceBasedHeuristic):
    ...
    def __call__(self, score):
        return sum(self._distances[(t.x, t.y)] for t in self._mp.board.get_tiles()) / len(self._mp.board.get_tiles()) + score
...
class WeightedDistanceSumAnchoringHeuristic(DistanceBasedHeuristic):
    ...
    def __call__(self, score):
        s = sum(self._distances[(t.x, t.y)] ** self._exponent for t in self._mp.board.get_tiles())
        return s
...
class DistanceToPolyominoAndTargetAreaHeuristic2:
    def __init__(self, mp):
        ...
        target_area_poly = \
            Polyomino(tiles=
                      (Tile(position=(x, y)) for x, y in reachable_set(self.mp.board, self.mp.target_shape))
                      )
        self._distances_to_target_area = compute_distances(self.mp.board, target_area_poly)
    ...
    def __call__(self, score):
        ...
        tile_distance_to_target_area = self._distances_to_target_area[self.mp.tile.x, self.mp.tile.y]
        return max(distance_to_target_area + tile_distance_to_target_area, min_moves) + score
...
class DistanceToPolyominoAndTargetAreaHeuristic:
    ...
    def pre_computation(board, target_shape):
        ...
        for (x, y) in reachable_set(board, target_shape):
            for dx, dy in target_shape.get_shape()[1]:
                DistanceToPolyominoAndTargetAreaHeuristic.target_area.add((x + dx, y + dy))
    ...
    def _extend_target_area(board):
        ...
        while active:
            current = active.popleft()
            if distance[current] > DistanceToPolyominoAndTargetAreaHeuristic.MAX_DISTANCE_TO_TARGET_AREA:
                continue
            for xy in direct_neighbors(*current):
    ...
    def __init__(self, mp):
        ...
        target_area_poly = \
            Polyomino(tiles=
                      (Tile(position=(x, y)) for x, y in target_area)
                      )
        self._distances_to_target_area = compute_distances(self.mp.board, target_area_poly)
...
class GreedyDistanceToPolyominoHeuristic:
    ...
    def __call__(self, score):
        ...
        dest = (poly_x + dx, poly_y + dy)
        distance = self.get_distance(tile_poly, dest)
        return int(distance / 2) + abs(self.mp.tile.x - dest[0]) + abs(self.mp.tile.y - dest[1])
...
class GreedyDistanceToFixedDestinationHeuristic:
    ...
    def _compute_distances(self):
        return breadth_first_distance({self.destination}, lambda xy: direct_neighbors(*xy),
...
def average_distance_to_center(board: Board, target_shape: Polyomino, score):
    ...
    while active:
        ...
        if target_shape.size <= tile_count:
            return sum(distance[t.x, t.y] for t in board.get_tiles()) / target_shape.size + score
        for xy in direct_neighbors(*current):
            if not board.is_blocked(*xy) and xy not in distance:
                distance[xy] = distance[current] + 1
                active.append(xy)
