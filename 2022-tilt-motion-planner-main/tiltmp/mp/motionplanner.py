...
class BFSMotionPlanner(MotionPlanner):
    ...
    def _initial_stop_condition(self):
        if self.target_shape.size == len(self.board.get_tiles()) and not hasattr(self.board, "fixed_tiles"):
    ...
    class NoLeftoversStopCondition:
        ...
        def extract_solution(self):
            if self.mp._solution_node is None:
                return None
            end_sequence = shortest_sequence(self.mp.board, self._solution_polyomino,
                                                                self.mp.target_shape.position)
        ...
        def is_finished(self):
            for polyomino in self.mp.board.polyominoes:
                if polyomino.shape_equals(self.mp.target_shape) and polyomino.position in self._target_area:
    ...
    class DefaultStopCondition:
        ...
        def is_finished(self):
            for polyomino in self.mp.board.polyominoes:
                if polyomino.position == self.mp.target_shape.position and polyomino.shape_equals(self.mp.target_shape):
                    return True
            return False
        ...
        def __init__(self, motion_planner):
            ...
            for t in self.mp.target_shape.get_tiles():
                if not self.mp.board.concrete[t.x, t.y]:
                    raise ValueError("target shape has to be concrete in order for anchoring to work")
        ...
        def is_finished(self):
            for polyomino in self.mp.board.adjacent_polyominoes(self.mp.target_shape):
                ...
                for direction in Direction:
                    ...
                    if not self.mp.board.polyomino_is_blocked(polyomino, direction, is_wall):
                        blocked = False
                        break
                if blocked:
                    return True
    ...
    def solve(self, max_nodes=None):
        ...
        while not self._active_nodes.empty() and self._solution_node is None and not self._stopped:
...
class HeuristicMotionPlanner(BFSMotionPlanner):
    def __init__(self, instance: Instance, heuristic=GreatestDistanceHeuristic, precomputed_distances=None):
        super().__init__(instance)
        self.score = {hash(self.board): 0.0}
        self._current_score = 0

        if precomputed_distances is not None and issubclass(heuristic, DistanceBasedHeuristic):
            self.heuristic = heuristic(self, precomputed_distances=precomputed_distances)
        else:
    ...
    def _expand_reachable_set(self, node):
        ...
        poly_all = Polyomino(tiles=[tile for poly in self.board.polyominoes for tile in poly.get_tiles()])
...
class PolyominoToAreaMotionPlanner(HeuristicMotionPlanner):
    def __init__(self, instance, poly, target_area):
        ...
        self._stop_condition = PolyominoToAreaMotionPlanner.PolyominoInAreaStopCondition(self)
        self.heuristic = DistancePolyominoToAreaHeuristic(self)
        self._active_nodes = PriorityQueue()
        self._active_nodes.put((self.heuristic(0),
...
class SingleTileMotionPlanner(HeuristicMotionPlanner):
    def __init__(self, instance: Instance, tile, poly, destination, offset,
    ...
    class TileAtDestination:
        ...
        def is_finished(self):
            ...
            return self.mp.tile.x == previous_corner_tile.x + self.mp.destination[0] and\
                   self.mp.tile.y == previous_corner_tile.y + self.mp.destination[1]
    ...
    def _compute_legal_poly_positions(self):
        ...
        next_poly = Polyomino(tiles=(
            Tile(position=(x + dx, y + dy)) for dx, dy in shape
        ))
    ...
    def _expand_reachable_set(self, node):
        ...
        for dx, dy, depth, n in expand_reachable_set(self.board, poly_all, node):
            ...
            self.score[h] = s
            n.state = self.board.get_state()
            n.polyomino_position = (node.polyomino_position[0] + dx, node.polyomino_position[1] + dy)
...
class OneTileAtATimeMotionPlanner(MotionPlanner):
    def __init__(self, instance: Instance, single_tile_heuristic=DistanceToPolyominoAndTargetAreaHeuristic):
        super().__init__(instance)
    ...
    def solve(self):
        ...
        while not self.build_order_planner.finished() and not self._stopped:
            ...
            (x, y), tile = self.build_order_planner.get_next_tile(self.poly, self.offset)
            destination = self._get_next_destination(x, y)
            assert tile is not None
            self.sub_motion_planner = SingleTileMotionPlanner(instance,
                                                            tile,
                                                            self.poly,
                                                            destination,
                                                            self.offset,
        ...
        self.solution += (d.value for d in shortest_sequence(self.board, self.poly, self.target_shape.position))
        return self.extract_solution()
...
def get_anchoring_motion_planner(instance: Instance):
    mp = HeuristicMotionPlanner(instance, heuristic=WeightedDistanceSumAnchoringHeuristic)
    anchoring = BFSMotionPlanner.AnchoringStopCondition(mp)
    mp.set_stop_condition(anchoring)
    return mp

def get_motion_planner(instance: Instance, heuristic=GreatestDistanceHeuristic, precomputed_distances=None):
    if instance.initial_state.number_of_tiles() == instance.target_shape.size:
        mp = HeuristicMotionPlanner(instance, heuristic=heuristic, precomputed_distances=precomputed_distances)
