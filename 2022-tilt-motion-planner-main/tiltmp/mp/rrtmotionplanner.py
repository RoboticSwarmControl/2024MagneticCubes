...
from tiltmp.mp.heuristic import WeightedDistanceSumHeuristic, DistanceBasedHeuristic, GreedyGreatestDistanceHeuristic
from tiltmp.mp.motionplanner import Instance, MotionPlanner, BFSMotionPlanner, get_motion_planner
...
class Configuration:
    ...
    def apply(self):
        ...
        if self.fixed_tiles_positions and hasattr(self.board, "fixed_tiles"):
            self.board.fixed_tiles = {self.tiles[position] for position in self.fixed_tiles_positions}
        self.board.activate_glues()

    def __hash__(self):
        return hash(tuple(sorted((tx, ty, t.glues) for (tx, ty), t in self.tiles.items())))
    ...
    def from_board(board):
        if hasattr(board, "fixed_tiles"):
            return Configuration(copy(board._tile_at), board, fixed_tiles=[(t.x, t.y) for t in board.fixed_tiles])
        return Configuration(copy(board._tile_at), board)


def get_random_configuration(board: Board):
    if hasattr(board, "fixed_tiles"):
        tiles = [tile for tile in board.get_tiles() if tile if tile not in board.fixed_tiles]
        seed_tile_positions = [(t.x, t.y) for t in board.fixed_tiles]
        open_positions = [(x, y) for x, y in np.argwhere(board.concrete == 0) if (x, y) not in seed_tile_positions]
    else:
...
def bottleneck_matching_old(g, top_nodes, max_iterations=10, max_bottleneck=float("inf")):
    # get weights
    ...
    while left < right and i < max_iterations:
        ...
        if perfect_matching_with_shorter_edges_exists(g, top_nodes, sorted_weights[mid]):
            left = mid + 1
        else:
            right = mid
        i += 1
...
def bottleneck_matching(g, top_nodes, max_iterations=10, max_bottleneck=float("inf")):
    ...
    while left < right and i < max_iterations:
        ...
        matching = perfect_matching_with_shorter_edges_exists(g, top_nodes, sorted_weights[mid])
        if matching:
            right = mid
...
def bottleneck_weighted_sum_matching(config1, config2, max_iterations=10, max_bottleneck=float("inf")):
...
def hausdorff_distance(config1, config2, max_bottleneck=float("inf"), distance_func=taxicab_distance):
    overall_max = 0
...
def bottleneck_edge_length(config1, config2, max_iterations=10, max_bottleneck=float("inf"),
...
def construct_matching_graph(config1, config2, distance_func):
    ...
    for (p1, t1), (p2, t2) in itertools.product(config1.tiles.items(), config2.tiles.items()):
        if t1.glues == t2.glues:
...
def bottleneck_edge_length_real_distance(config1, config2, max_iterations=10, max_bottleneck=float("inf")):
...
class RRTSolver(MotionPlanner):
    def __init__(self, instance, bias=0.05, distance_metric=hausdorff_distance, max_nodes=None):
        super().__init__(instance)
        self._target_distances = compute_distances(self.board, self.target_shape)
        _compute_distance_map(self.board)
        self._target_shape_positions = [(t.x, t.y) for t in self.target_shape.get_tiles()]
    ...
    def expand_towards_config(self, node, config, max_iterations=40):
        node.config.apply()
        solver = get_motion_planner(Instance(self.board, self.target_shape),
    ...
    def find_closest_node(self, config):
        ...
        selected_node = None
        for node in self.nodes:
            distance = self.get_config_distance(node.config, config)#, max_bottleneck=min_distance)
    ...
    def get_config_distance(self, config1, config2, max_bottleneck=float("inf")):
        if not self.polyominoes_match(config1, config2):
            return float("inf")
        return self.distance_metric(config1, config2, max_bottleneck=max_bottleneck,
    ...
    def distance_to_target(self, config, max_distance=float("inf")):
        ...
        sorted_tiles = list(sorted(((self._target_distances[position], tile)
        ...
        for i in range(start_index, len(sorted_tiles) + 1):
            ...
            if get_blueprint_with_glue_types(self._target_shape_positions, tiles,
                                          glue_rules=self.board.glue_rules):
                return distance
        return float("inf")
    ...
    def _get_stop_condition(self):
        if self.max_nodes is None:
            ...
            def stop():
                return self._solution_node or self._stopped or len(self.nodes) >= self.max_nodes
        return stop

    def is_finished(self):
        for polyomino in self.board.polyominoes:
            if polyomino.shape_equals(self.target_shape) and polyomino.position in self._target_shape_positions:
                return True
        return False
