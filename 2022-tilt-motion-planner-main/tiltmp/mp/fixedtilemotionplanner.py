...
class OriginalBoardGenerator:
    ...
    def reverse_step_direction(self, direction):
        stack = deque()
        stack.append(StackFrame(set(self.board.get_tiles()),  self.original_state, self.board.fixed_tiles))
        ...
        while stack:
            ...
            for c in combinations(can):
                ...
                self.board.polyominoes = [Polyomino(tiles=[tile]) for tile in self.all_tiles]
                ...
                if any(tile in moving for tile in self.fixed_poly.get_tiles()):
                    ...
                    stack.append(StackFrame(new_not_moved, self.board.get_state(), decided_fixed))
    ...
    def must_can_move(self, stack_frame: StackFrame, direction):
        ...
        for tile in stack_frame.not_moved:
            ...
            if temp not in stack_frame.decided_fixed and not board.is_blocked(temp.x + rdx, temp.y + rdy):
                can_move.add(temp)
        ...
        for tile in stack_frame.not_moved.intersection(self.fixed_poly.get_tiles()):
            ...
            if tile in board.fixed_tiles:
                continue
            if tile not in stack_frame.decided_fixed and not board.is_blocked(tile.x + rdx, tile.y + rdy):
                can_move.add(tile)
