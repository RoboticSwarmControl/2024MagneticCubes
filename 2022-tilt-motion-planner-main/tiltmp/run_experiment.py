...
__all__ = ["SolutionData", "profile"]

from tiltmp.mp.rrtmotionplanner import RRTSolver


class SolutionData:
    def __init__(self, control_sequence, time_needed, runtime_profile="", instance=None, number_of_nodes=0):
        if instance:
...
def main():
    parser = argparse.ArgumentParser(description='Solve instance of polyomino construction problem')
    parser.add_argument('input', metavar="IN", type=str,
                        help='file path to an instance or directory')
    parser.add_argument("--out", "-o", type=str, metavar="OUT",
                        default=None,
                        help="output file path for the profiling results")
    parser.add_argument("--outdir", type=str, metavar="OUT",
                        default=None,
    ...
    parser.add_argument("--solver", "-s", type=str, metavar="SOLVER",
                        default="default",
                        help="Solver to be used")
    parser.add_argument("--heuristic", type=str, metavar="SOLVER",
    ...
    if args.out is None and args.outdir is None:
        args.out = os.path.join("results", os.path.splitext(os.path.basename(args.input))[0] + "_results.json")
    ...
    if os.path.isdir(args.input):
        ...
        run_multiple_experiments(filenames, args.outdir, args.solver, args.heuristic, timeout=args.timeout)
        return
    ...
    if args.out:
        ...
        output_file = os.path.join(args.outdir, os.path.splitext(os.path.basename(args.input))[0] + "_result.json")


    try:
        run_experiment(args.input, output_file, args.solver, args.heuristic, timeout=args.timeout, p=args.profile)
...
def run_experiment(input_file, output_file, solver, heuristic, timeout=None, p=False):
    ...
    results = profile(instance, solver, heuristic, timeout=timeout) if p \
...
def run_multiple_experiments(input_files, output_folder, solver, heuristic, timeout=None):
    for file in input_files:
        print(file)
        output_file = os.path.join(output_folder, os.path.splitext(os.path.basename(file))[0] + "_result.json")
        try:
...
def profile(instance: Instance, solver_name, heuristics, timeout=None):
    ...
    try:
        ...
        nn = 0
    data = SolutionData(solution, time_needed, runtime_profile=stats, instance=instance, number_of_nodes=nn)
    data.timed_out = timed_out
    return data
