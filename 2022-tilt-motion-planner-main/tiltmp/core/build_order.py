...
from tiltmp.core.algorithms import is_connected_by_glues, is_connected, compute_distances, breadth_first_path_exists, \
    find_largest_glue_connected_component
from tiltmp.core.tumbletiles import *


class BuildOrderPlanner:
    def __init__(self, board: Board, target_shape: Polyomino, precomputed_distances=None, timeout=600,
        ...
        if hasattr(self._board, "fixed_tiles"):
            ...
            self._fixed = [(t.x - offset[0], t.y-offset[1]) for t in self._board.fixed_tiles]
        else:
            self._fixed = []

        self.change_build_order()

        self._distances = precomputed_distances or compute_distances(self._board, self._target_shape)
    ...
    def change_build_order(self):
        ...
        while True:
            ...
            if self._glues is None:
                ...
                self._glues = get_blueprint_with_glue_types(shape, tiles, self._board.glue_rules,
                                                            fixed_tiles=self._board.fixed_tiles
                                                            if hasattr(self._board, "fixed_tiles") else [],
                                                            offset=self._target_shape.position,
            ...
            if hasattr(self._board, "fixed_tiles"):
                self._build_order_generator = FixedTilesBuildOrderFinder(self._glues, self._board.glue_rules, self._board, self._target_shape,
                                           end_time=self.end_time,
                                           max_time_per_result=self._build_order_timeout)
            else:
                self._build_order_generator = BuildOrderFinder(self._glues, self._board.glue_rules, self._fixed,
                                                               end_time=self.end_time,
                                                               max_time_per_result=self._build_order_timeout)
    ...
    def get_next_tile(self, current_poly, offset):
        ...
        relative_to_poly = (next_destination[0] - offset[0], next_destination[1] - offset[1])
        next_glue_type = self._glues[next_destination]
        poly = current_poly if current_poly is not None else self._target_shape
        absolute_destination = poly.position[0] + relative_to_poly[0], poly.position[1] + relative_to_poly[1]
        return next_destination, \
               self._find_nearest_tile_with_glue_type(poly, {absolute_destination}, next_glue_type)
...
def get_blueprint_with_glue_types(container: Iterable[tuple], tiles: Iterable[Tile], glue_rules
                                  , fixed_tiles=(), offset=(0, 0), timeout=600.0):
    ...
    while True:
        if time.time() - start_time > timeout:
            return None
        result = _get_blueprint_with_glue_types_recursive(container, current_shape, available_glue_types, glue_rules)
        if result:
            return result



def _get_blueprint_with_glue_types_recursive(missing: List[tuple], current_shape, available_glue_types, glue_rules):
    if not missing:
    ...
    for glue in glue_types:
        if available_glue_types[glue] > 0:
            ...
            return _get_blueprint_with_glue_types_recursive(missing[:-1], new_shape, remaining_glues, glue_rules)
    return None
...
def remove_path_exists(glues: Dict[tuple, Glues], tile_position, glue_rules: GlueRules):
    ...
    return breadth_first_path_exists({tile_position}, lambda p: p == outside,
                                     lambda p: direct_neighbors(*p),
                                     is_valid_neighbor=lambda p: p not in blocked_positions)


def remove_path_exists_fixed(glues: Dict[tuple, Glues], tile_position, glue_rules: GlueRules, board: Board, outside):
    blocked_positions = set()
    tile_glues = glues[tile_position]
    for xy, g in glues.items():
        if xy == tile_position:
    ...
    return breadth_first_path_exists({tile_position}, outside,
                                     lambda p: direct_neighbors(*p),
                                     is_valid_neighbor=lambda p: p not in blocked_positions and not board.is_blocked(*p))

def is_removable(old_shape, new_shape, tile_position, glue_rules):
    return is_connected_by_glues(new_shape, glue_rules) \
           and remove_path_exists(old_shape, tile_position, glue_rules)


def is_removable_fixed(old_shape, new_shape, tile_position, glue_rules, board, outside):
    return is_connected_by_glues(new_shape, glue_rules) \
           and remove_path_exists_fixed(old_shape, tile_position, glue_rules, board, outside)


class BuildOrderFinder:
    def __init__(self, glues: Dict[tuple, Glues], glue_rules, fixed_tiles_positions, end_time=None, max_time_per_result = 10.0):
    ...
    def _check_timeouts(self):
        ...
        if self._next_result_timeout is not None and time.time() > self._next_result_timeout:
    ...
    def _generate_building_orders_recursive(self, order, glues: Dict[tuple, Glues]):
        ...
        s = copy(glues)
        for t in s:
            if len(s) > len(self.fixed_tiles_positions) and t in self.fixed_tiles_positions:
                continue
            new_shape = copy(s)
            new_shape.pop(t)
...
class FixedTilesBuildOrderFinder(BuildOrderFinder):
    def __init__(self, glues: Dict[tuple, Glues], glue_rules, board: FixedSeedTilesBoard, target_shape: Polyomino, end_time=None,
                 max_time_per_result=10.0):
        fixed_tiles_positions = [(t.x, t.y) for t in board.fixed_tiles]
        super().__init__(glues, glue_rules, fixed_tiles_positions, end_time=end_time,
    ...
    def _generate_building_orders_recursive(self, order, glues: Dict[tuple, Glues]):
        ...
        for t in s:
            ...
            new_shape = copy(s)
            new_shape.pop(t)
            if not is_removable_fixed(s, new_shape, t, self.glue_rules, self.board, self.outside):
                continue
            new_order = order + [(t[0] - self.dx, t[1] - self.dy)]
            for o in self._generate_building_orders_recursive(new_order, new_shape):
                yield o

def generate_building_orders(glues: Dict[tuple, Glues], glue_rules, fixed_tiles_positions, end_time):
    return _generate_building_orders_recursive([], glues, glue_rules, fixed_tiles_positions, end_time)


def _generate_building_orders_recursive(order, glues: Dict[tuple, Glues], glue_rules, fixed_tiles_positions, end_time):
    ...
    for t in s:
        ...
        if not is_removable(s, new_shape, t, glue_rules):
            continue
        new_order = order + [t]
        for o in _generate_building_orders_recursive(new_order, new_shape, glue_rules, fixed_tiles_positions, end_time):
            yield o
