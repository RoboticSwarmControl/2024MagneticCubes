...
def random_instance(random_board_function, board_dimensions, target_shape_size, leftover_tiles=0, glue_number=1, fixed_seed_tile=False):
    ...
    tiles, rules = create_random_glues(target_shape, glue_number,
                                       leftover_tiles)
    ...
    if fixed_seed_tile:
        ...
        for tile in tiles:
            if tile.glues == fixed_seed_tile.glues and tile.x==fixed_seed_tile.x and tile.y==fixed_seed_tile.y:
...
def place_tiles_randomly(board, tiles):
    while tiles:
        ...
        if board.is_occupied(x, y) or any(board.get_tile_at(nx, ny) for nx, ny in direct_neighbors(x, y)):
            continue
        t.x, t.y = x, y
        p = Polyomino(tiles=[t])
        board.add(p)
...
def is_valid_target_shape_position(board, target_shape):
    if board.fits(*target_shape.get_shape()) and len(reachable_set(board, target_shape)) > 1:
        return True
    else:
        return False
...
def create_random_glues(shape, number_glue_types, additional_tiles):
    ...
    def rand_glues():
        return Glues(random.choice(glues), random.choice(glues),
                     random.choice(glues), random.choice(glues))
...
if __name__ == '__main__':
    ...
    for instance_type in itertools.product(BOARD_TYPE,TILES,SIZE,LEFTOVER,GLUES,PROBLEM):
        board_type, tiles, size, leftover, glues, fixed = instance_type
        f = gridcreation.random_maze_board if board_type == "maze" \
        ...
        for i in range(1, 6):
            ...
            instance = random_instance(f, (size, size), tiles, leftover, glues,
                                       fixed_bool)
