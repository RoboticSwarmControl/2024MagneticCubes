...
class CellularRules:
    ...
    def apply(self, grid):
        ...
        for (x, y), alive in np.ndenumerate(current_grid):
            grid[x, y] = self._get_next_state(alive, self._count_neighbors(current_grid, x, y))
...
def generate_caves(size, cellular_rules=CellularRules(lambda ln: ln >= 5, lambda ln: ln < 4), p=0.45):
    while True:
...
def generate_maze_with_open_areas(size):
    ...
    for _ in range(int(size_factor / 2)):
        ...
        maze[position_x: position_x + size_x, position_y:position_y + size_y] \
...
def random_maze_board_no_chambers(size):
    ...
    delete = sample([(x, y) for x, y in np.argwhere(concrete_positions == 1)], k=int(size[0] * size[1] / 12))
...
def noise_array(dim, octaves):
    ...
    for (x, y), _ in np.ndenumerate(noise):
        for p in parameters:
            offset = math.sin(x / dim[0] * p["frequency"]
                              * 2 * math.pi
                              + p["offset"]
                              )
            noise[x, y] += math.sin(y / dim[1] * p["frequency"]
                                    * 2 * math.pi
                                    + offset
                                    ) * p['amplitude']
    return noise
