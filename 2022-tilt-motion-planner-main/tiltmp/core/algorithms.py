...
def breadth_first_reachable(start, neighbors, is_valid_neighbor=lambda n: True):
    return breadth_first_distance(start, neighbors, is_valid_neighbor=is_valid_neighbor).keys()
...
def breadth_first_path_exists(start, goal_condition, neighbors, is_valid_neighbor=lambda n: True):
    distance = {t: 0 for t in start}
    active = deque(distance.keys())
    while active:
        current = active.popleft()
...
def find_shortest_path(board: Board, polyomino: Polyomino, target_position: tuple):
    ...
    while not queue.empty():
        ...
        for xy in direct_neighbors(*current):
            if board.fits(xy, rel_coord) and tentative_g_score < g_score.get(xy, float("inf")):
                paths[xy] = current
...
def shortest_sequence(board: Board, polyomino: Polyomino, target_position: tuple):
    return find_shortest_path(board, polyomino, target_position).get_moves(target_position)
...
def is_connected_by_glues(glues: Dict[tuple, Glues], rules: GlueRules):
    ...
    while active:
        ...
        for d in Direction:
            ...
            if rules.sticks(getattr(current_glues, d.value),
...
def direct_path_between_configs(b: Board, state1, state2):
    ...
    poly1 = Polyomino(tiles=[Tile(position=xy, glues=glues) for xy, glues in position_to_glues1.items()])
    poly2 = Polyomino(tiles=[Tile(position=xy, glues=glues) for xy, glues in position_to_glues2.items()])
