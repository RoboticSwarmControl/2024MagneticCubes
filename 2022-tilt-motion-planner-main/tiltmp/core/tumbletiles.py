...
class Tile:
    def __init__(self, color="#FFFF00", position=(0, 0), glues=Glues(None, None, None, None), is_concrete=False):
...
class Polyomino:
    ...
    def can_join(self, poly, rules):
        ...
        for t in self.get_tiles():
            for pt in poly.get_tiles():
                ...
                if t.x - pt.x == 1 and t.y == pt.y and rules.sticks(t.glues.W, pt.glues.E):
                    return True
                # t on left, pt on right
                if pt.x - t.x == 1 and t.y == pt.y and rules.sticks(t.glues.E, pt.glues.W):
                    return True
                # t on top, pt on bottom
                if t.x == pt.x and t.y - pt.y == -1 and rules.sticks(t.glues.S, pt.glues.N):
                    return True
                # pt on top, t on bottom
                if t.x == pt.x and pt.y - t.y == -1 and rules.sticks(t.glues.N, pt.glues.S):
                    return True
    ...
    def __str__(self):
        return "Polyomino at " + str(self.position) +\
               ", with tiles: " + str(self.tiles.keys())
...
class Board:
    ...
    def copy(self, selection=None):
        ...
        if selection is None:
            ...
            return deepcopy(self.coord_to_tile[selection[0]: selection[1], selection[2]: selection[3]])
    ...
    def resize_board(self, w, h):
        ...
        print(h, w)
        new_concrete[0: min(self.concrete.shape[0], h), 0: min(self.concrete.shape[1], w)] \
            = self.concrete[0: min(self.concrete.shape[0], h), 0: min(self.concrete.shape[1], w)]
    ...
    def _recursively_find_blocked_polyominoes(self, p, direction):
        ...
        while active:
            ...
            for neighbor in self.adjacent_polyominoes_in_direction(current, direction.inverse()):
    ...
    def get_state(self):
        return tuple(self._tile_at.items()), tuple(p.get_state() for p in self.polyominoes)
...
class FixedSeedTilesBoard(Board):
    ...
    def step(self, direction):
        ...
        for p in self.polyominoes:
            if (p in free_to_move and self.polyomino_is_blocked(p, direction, is_wall)) or p in fixed_polyominoes:
                blocked = self._recursively_find_blocked_polyominoes(p, direction)
                free_to_move.difference_update(blocked)
