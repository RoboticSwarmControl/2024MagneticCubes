...
class GlueRulesEncoder(json.JSONEncoder):
    ...
    def encode_rules(glue_rules):
        return {"rules": list(list(rule) for rule in glue_rules.rules),
...
def decode_board(data):
    ...
    if "height" not in data.keys():
        return data
    board = board_class(data["height"], data["width"],
                        glue_rules=decode_glue_rules(data["glueRules"]))
    ...
    if board_class is FixedSeedTilesBoard:
        ...
        board.fixed_tiles = {t for t in board.get_tiles() if (t.x, t.y) in fixed_tile_positions}
...
class InstanceEncoder(json.JSONEncoder):
    ...
    def encode_instance(instance):
        data = {"board": BoardEncoder.encode_board(instance.initial_state),
...
def decode_instance(data):
    if "board" not in data.keys():
        return data
    instance = Instance(decode_board(data["board"]), decode_polyomino(data["target_shape"]))
    return instance
