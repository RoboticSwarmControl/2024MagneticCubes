...
\begin{figure}
	...
	\caption[Time-use for certain steps in simulation loop]{Fraction of time used on certain steps in the simulation loop. The simulator ran for $8$ seconds without drawing and executed various motions with ten cubes in the workspace. The steps can be found in the flowchart diagram of \autoref{fig:simulator}.}
	\label{fig:timeuse}
\end{figure}
...
After the Pymunk-step the magnetic forces between the cube's permanent magnets are calculated (\autoref{sec:force_magnet}).
...
Because assembling large target structures is the goal of global planning, it is not uncommon that most cubes are in critical-distance.
