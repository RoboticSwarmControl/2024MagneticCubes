...
\begin{equation}
F_\textit{magnet} = \frac{\mu_\textit{magnet}}{r^4} (m_2(m_1 \cdotp \hat{r}) + m_1(m_2 \cdotp \hat{r}) + \hat{r}(m_1 \cdotp m_2) - 5\hat{r}(m_1 \cdotp \hat{r})(m_2 \cdotp \hat{r})) \, ,s
\end{equation}
with $\hat{r}$ being the unit vector pointing from magnet 1 to magnet 2.
The force acting from magnet 2 on magnet 1 is simply $-F_\textit{magnet}$.
We set the strength of all magnets to $\mu_{magnet} = 25000000$ \cite{levitt2013}. 
