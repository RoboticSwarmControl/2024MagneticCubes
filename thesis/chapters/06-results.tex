...
Random polyominoes and initial configurations are created with a seed-based pseudorandom number generator to make experiments reproducible.
...
We conducted the same experiment for seed $100$ with a altered local planner that develops local plans sequentially and returns the one with the lowest cost.
...
We use the benefit of parallel processing to reduce planning time in further experiments, even thought we are aware of the non-determinism it causes.
...
When comparing planning time of the two cube patterns in \autoref{fig:AFTS_cb_time}, the checkerboard pattern performs worst for all types of rectangular polyominoes.
