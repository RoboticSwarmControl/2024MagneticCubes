...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\LOOP
			...
			\ENDIF
			\IF{stuck}
				\STATE $a \gets \text{\scshape Wait-Until-Connected}(c_A, c_B, e_A)$
			\ELSIF{$\text{\scshape Critical-Distance}(c_A, c_B)$ \AND wait}
				\STATE $a \gets \text{\scshape Wait}()$
				\STATE $\text{wait} \gets$ \FALSE
			\ELSE
				\STATE $a \gets \text{\scshape Walk-Approximate}(c_A, c_B, \vec{w})$
				\STATE $\text{wait} \gets$ \TRUE
			\ENDIF
			\STATE $g_{goal} \gets \text{\scshape Simulate}(g_{goal}, a)$
			\STATE $A \gets \text{\scshape Append}(A, a)$
