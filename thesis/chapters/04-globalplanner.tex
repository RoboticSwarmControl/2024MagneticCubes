...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\WHILE[work through nodes in BFS manner]{$i < \text{\scshape Size}(V)$}
			\STATE $S_i \gets V[i]$
			\FOR[go through all polyomino types in $S_i$]{$\forall A \in S_i$}
				\FOR[go through all monotone two-cuts]{$\forall tc \in \text{\scshape Two-Cuts}(A)$}
					\STATE $A_1$,$A_2 \gets \text{\scshape Cut-Polyomino}(A, tc)$
					\STATE $S_{new} \gets S_i \setminus \{A\} \cup \{A_1, A_2\}$ \COMMENT{new node after cutting}
					\IF{$S_{new} \notin V$}
...
The cutting results in the two sub-polyomino $A_1$ and $A_2$.
$S_{new}$ contains the same polyominoes as $S_i$ with the exception, that one occurrences of $A$ got removed and replaced by one occurrence of each $A_1$ and $A_2$.
...
\begin{figure}
	\centering
	\includegraphics[width=0.9\textwidth]{figures/plots/tcsa_nodes_edges.pdf}
...
It will be interesting to see how these complementary methods perform.
		...
		\LOOP
			...
			\ELSE
				\IF[no configuration to fall back to]{$\text{\scshape Empty}(P)$}
					...
					\RETURN $s$, $P$
				\ENDIF
				\STATE $p_{pre} \gets \text{\scshape Pop}(P)$ \COMMENT{remove last plan form plan stack}
...
Because a TCSA graph was used to create the local plans, $|P| < n$ holds true (\autoref{sec:local_in_global}).
